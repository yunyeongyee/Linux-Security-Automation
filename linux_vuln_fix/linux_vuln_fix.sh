...
# [분리본 1/2] U-01 ~ U-67 (계정/파일·디렉터리/서비스/패치/로그 관리)

# [보호 규칙] 요약 대시보드 UI/그래프 생성 로직은 사용자 명시 요청 없이 수정 금지.
...
# ── UI 헬퍼 함수 (조기 정의) ───────────────────────────────────────────────────
# 구분선/라벨/섹션/행 헬퍼는 원래 스크립트 하단(구 1450~1730줄대)에 있었으나,
# --rollback 조기 분기(스캔/조치 로직 진입 전에 먼저 처리되는 블록)가 이 함수들을
# 먼저 호출해서 "command not found"로 조용히 실패하는 순서 버그가 있었다
# (exec 9>lock 2>/dev/null 이 이후 stderr를 전부 삼켜서 지금까지 드러나지 않았음).
# 색상 코드 정의 외에 다른 의존성이 없는 함수들이라 안전하게 맨 앞으로 옮긴다.
_div_item() {
  echo -e "${WHITE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
}
_div_sec() {
...
FIXED=0; SKIPPED=0; FAILED=0; MANUAL=0; NA=0; DRYRUN=0
FIXED_LIST=(); SKIPPED_LIST=(); FAILED_LIST=(); MANUAL_LIST=(); NA_LIST=(); DRYRUN_LIST=()
...
# /linux_vuln_fix/
# ├── backup/     사전 백업 tar.gz
# ├── report/     CSV, XLSX, TXT, 누적 vulnFixHistory.log
# └── rollback/   롤백 실행 로그·검증 로그·정책 파일
_BASE_DIR="/linux_vuln_fix"
# 백업/보고서/롤백 실행 산출물은 역할별 공통 디렉터리에 저장한다.
...
# 누적 실행 이력은 사용자가 결과 보고서와 함께 확인할 수 있도록 report에 저장한다.
# 기존 버전의 /rollback/vulnFixHistory.log가 있으면 최초 실행 시 안전하게 이전한다.
_OLD_FIX_HISTORY_FILE="${_RB_DIR}/vulnFixHistory.log"
FIX_HISTORY_FILE="${_RPT_BASE_DIR}/vulnFixHistory.log"

_vf_migrate_history_file() {
  local _old="$_OLD_FIX_HISTORY_FILE" _new="$FIX_HISTORY_FILE"
  [ "$_old" = "$_new" ] && return 0
  [ -f "$_old" ] || return 0
...
if ! _vf_migrate_history_file; then
  echo -e "${RED}[오류] 기존 vulnFixHistory.log를 report 디렉터리로 이전하지 못했습니다.${RESET}"
  echo -e "${YELLOW}       기존 파일은 삭제하지 않았습니다: ${_OLD_FIX_HISTORY_FILE}${RESET}"
  exit 1
fi
...
# 이 실행의 고유 식별자 — 롤백 자동 역산 매칭용.
# (과거에는 백업 파일명의 시각을 검색키로 썼는데, 파일명 시각은 백업 시점에 date를
#  다시 호출해 만들어져 _RUN_TS와 거의 항상 달랐고, 그 결과 역산 레코드를 찾지 못했다.
#  이제 백업 파일명도 _RUN_TS를 그대로 쓰고, 이력 매칭은 BAK=<전체 경로>를 1순위로 한다.)
_RUN_ID="${_RUN_TS}_$$"
...
# =============================================================================
# ── [Rollback v3 메타데이터/복원 보강] ────────────────────────────────────────
# 요약 대시보드 UI 및 그래프 생성 로직과 독립된 롤백 전용 공통 함수다.
# =============================================================================
...
# 백업·로그 보존 정책: 실행할 때마다 타임스탬프가 붙은 새 파일이 계속 생기고
# 지우는 로직이 없어 디스크가 무한히 채워질 수 있다(실제로 "No space left on
# device"로 롤백 직전 안전 백업이 실패한 사례가 있었다). 새 파일을 만들기 전에
# 오래된 것부터 정리해 최근 N개만 유지한다. 개수는 환경변수로 조정 가능.
...
_vf_prune_old_artifacts() {
  # _vf_prune_old_artifacts <디렉터리> <glob 패턴> <유지 개수> <표시 이름>
  # 대상 디렉터리에서 패턴에 맞는 파일을 mtime 최신순으로 정렬해 <유지 개수>개만
  # 남기고 나머지와 그 사이드카(.sha256 / .records)를 함께 삭제한다.
...
_vf_meta_value() {
  # _vf_meta_value <manifest> <key>
  awk -F'|' -v k="$2" '$1==k {sub(/^[^|]*\|/, ""); print; exit}' "$1" 2>/dev/null
}


# P7: 검증 명령의 오류 출력을 실행 환경에 따라 달라지는 값만 제거해 정규화한다.
# 구버전 VERIFY_BASELINE 레코드에는 해시가 없으므로 기존 판정 방식과 호환된다.
...
_vf_verify_output_sha256() {
  # _vf_verify_output_sha256 <원문>
  command -v sha256sum >/dev/null 2>&1 || return 1
  printf '%s' "$1" | _vf_normalize_verify_output | sha256sum | awk '{print $1}'
}


# 설정 검증 기준값을 지정한 레코드 파일에 저장한다.
# pre_rollback 백업도 기준값을 보유해야 기존 실패 상태를 롤백 실패로 오판하지 않는다.
_vf_capture_verify_baselines() {
  # _vf_capture_verify_baselines <records-file> <workdir>
...
# P2: 선택한 백업에 대응하는 한 실행의 롤백 레코드를 로드한다.
# 전체 경로 → 파일명 → 실행 시각 순으로 매칭해 백업 디렉터리 이동도 허용한다.
_vf_extract_run_records() {
  # _vf_extract_run_records <source-file> <selected-backup> <backup-basename> <run-ts>
...
# P2: 현재 실행의 역산 레코드를 백업 옆 사이드카로 원자적으로 저장한다.
_vf_export_run_records_sidecar() {
  # _vf_export_run_records_sidecar <backup-file>
...
_vf_normalize_firewalld_dump() {
  # runtime 출력의 zone 헤더에는 "(active)"가 붙을 수 있으므로 제거한 뒤
  # 공백·순서 차이를 정규화해 permanent 설정과 의미 기준으로 비교한다.
  sed -E \
...
_vf_capture_path_inventory() {
  # _vf_capture_path_inventory <meta-dir> <backup-root...>
  local _dir="$1"; shift
  local _root _p
...
_vf_capture_runtime_meta() {
  # _vf_capture_runtime_meta <meta-root> <backup-target...>
  local _root="$1"; shift
...
_vf_normalize_text_file() {
  # 주석·공백·출력 순서 차이로 인한 오탐을 줄이기 위한 비교용 정규화
  sed -e 's/[[:space:]]\+$//' -e '/^[[:space:]]*$/d' "$1" 2>/dev/null | LC_ALL=C sort
}

# P4: 실제 롤백 직전에 현재 상태를 별도 안전 백업으로 보존한다.
# 선택한 백업의 복원 대상과, 이력 레코드상 추가로 변경될 수 있는 경로만 정확히 담는다.
# 결과 전역:
...
_vf_create_pre_rollback_backup() {
  # _vf_create_pre_rollback_backup <selected-backup> <restore-file-list> <workdir> <rb-log> <verify-log>
  ...
  # 새 안전 백업을 만들기 전에 오래된 것부터 정리한다 — "No space left on device"로
  # 이 백업 자체가 실패하는 상황을 예방하기 위한 조치라 tar 실행 직전에 수행한다.
...
_vf_compare_packages_after_rollback() {
  # 결과 전역: _VF_PKG_ADDED/_REMOVED/_CHANGED/_MANUAL
...
_vf_validate_accounts_after_rollback() {
  # 결과 전역: _VF_ACCOUNT_OK/_FAIL/_MANUAL
  local _baseline="$1" _workdir="$2" _log="$3" _verify="$4"
...
_vf_restore_recorded_paths() {
  # 결과 전역: _VF_PATH_OK/_FAIL/_MANUAL
  ...
  if [ -f "$_baseline" ]; then
    # 구버전 및 pre_rollback 메타데이터가 미정렬이어도 comm 오집계를 방지한다.
...
_vf_restore_orphan_owners() {
  # U-15(무소유 파일) 전용 롤백 복원. 결과 전역: _VF_ORPHAN_OK/_FAIL/_MANUAL
  # 일반 PERM_RESTORE와 분리한 이유:
  #  - U-15는 mode를 변경하지 않으므로 mode는 검증 전용(드리프트 감지)이며 복원하지 않는다.
  #  - UID/GID가 백업 시점 이후 다른 계정에 재사용됐으면 절대 숫자로 강제 복원하지 않는다.
  #  - 소유자만 무소유였는지/그룹만 무소유였는지를 구분해, 조치가 실제로 바꾼 축만 복원한다.
...
# 롤백 후 설정 반영 대상 서비스의 설정 파일이 실제 복원 목록에 포함됐는지 확인한다.
# SSH는 세션 보호를 위해 이 대상에 포함하지 않는다.
_vf_rb_service_config_restored() {
  # _vf_rb_service_config_restored <systemd-unit> <tar-file-list>
...
# P3 대상 서비스는 복원 설정의 안전한 검증이 끝나기 전에 비활성 상태에서 자동 시작하지 않는다.
# 이미 active인 서비스만 아래 _vf_apply_restored_service_configs에서 검증 후 reload/restart한다.
_vf_restore_service_states() {
  # 결과 전역: _VF_SERVICE_OK/_FAIL/_MANUAL
...
# P3: 복원된 서비스 설정을 실행 중 서비스에 안전하게 반영한다.
# 원칙: 설정 파일 복원 + 서비스 active + 안전한 config test 통과 시에만 반영한다.
#       SSH는 자동 reload/restart하지 않는다.
# 결과 전역: _VF_CONFIG_APPLY_OK/_MANUAL/_SKIP
_vf_apply_restored_service_configs() {
  # _vf_apply_restored_service_configs <tar-file-list> <workdir> <rb-log> <verify-log>
...
_vf_normalize_iptables_dump() {
  # 규칙 순서는 유지하고 실행 중 변하는 카운터·생성시각 주석만 정규화한다.
  sed -E \
    -e 's/\[[0-9]+:[0-9]+\]/[0:0]/g' \
    -e '/^# Generated by /d' \
    -e '/^# Completed on /d' \
...
_vf_compare_extended_one() {
  # 결과 전역 누적: _VF_EXT_OK/_FAIL. 지원되지 않는 기능은 파일별 실패가 아니라
  # 카테고리 단위 추가확인으로 집계해 오탐을 줄인다.
...
# _report_add <id> <최종결과> [수동확인사유] [실패사유]
# 최종결과는 반드시 다음 중 하나: 양호 | 조치완료 | 수동확인 | 실패 | 해당없음 | 건너뜀
_report_add() {
...
# 실행 대상 중 결과 행이 누락된 항목은 조용히 빠뜨리지 않고 "실패"로 명시한다.
# 이렇게 해야 엑셀의 전체 진단 항목 수와 실제 TARGET_IDS 수가 항상 일치한다.
...
for _a in "$@"; do
  case "$_a" in
    --dry-run)
      echo -e "${RED}[오류] --dry-run 기능은 현재 지원하지 않습니다.${RESET}"
      exit 1
      ;;
    ...
    -h|--help)
      ...
      echo -e "${BOLD} Linux 취약점 자동 점검 및 조치 스크립트${RESET}"
      echo -e " KISA 2026 가이드 기반 · U-01 ~ U-67"
      echo -e "${BOLD}${WHITE}==================================================================${RESET}"
      echo ""
...
# DRY-RUN은 화면에서 변경 계획만 확인하는 모드이다.
# TXT/CSV/XLSX/백업 파일을 만들지 않으므로 별도의 _DRYRUN 결과 파일명도 생성하지 않는다.

# ── Rollback 조기 분기 ───────────────────────────────────────────────────────
# --rollback은 점검/스캔/사전백업/조치 로직으로 진입하면 안 된다.
# 옵션 파싱 직후 여기서 먼저 처리해야 실제 조치 프롬프트가 뜨지 않는다.
# (예전에는 이 자리에 별도의 _do_rollback_early 구현이 있었고, 스캔 로직 이후에
#  똑같은 내용의 _do_rollback이 또 있었다 — 후자는 조기 분기가 먼저 exit 0으로
#  끝내버려 항상 도달 불가능한 죽은 코드였다. 지금은 하나로 합쳤다.)
# ./linux_vuln_fix_report.sh --rollback
# 동작: /linux_vuln_fix/backup/vulnFix_backup_*.tar.gz 목록 표시 → 선택 → 복원
_do_rollback() {
  _div_thick
  ...
  # 1) 백업 파일 목록 수집
  #    일반 사전 백업과 P4 롤백 직전 안전 백업을 함께 표시한다.
  ...
  for _f in "${_bak_files[@]}"; do
    ...
    # 다른 서버 백업 여부와 백업 유형을 함께 표시한다.
    case "$(basename "$_f")" in
      pre_rollback_*) _type_mark="" ;;
      *) _type_mark="" ;;
    esac
    case "$(basename "$_f")" in
    ...
    printf "   %2d) %s  [%s]  %s" "$i" "$_ts_fmt" "$_sz" "$(basename "$_f")"
    echo -e "${_type_mark}${_host_mark}"
    i=$((i+1))
  done
  echo ""
  ...
  # 백업 SHA-256 검증. 구버전 백업처럼 체크섬이 없으면 실패가 아니라 추가 확인으로 분류한다.
  if command -v sha256sum >/dev/null 2>&1; then
  ...
  if [ -f "$_manifest" ]; then
    ...
    # 구버전 백업은 범위 필드가 없을 수 있으므로 차단하지 않는다.
    # 값이 존재하면서 현재 스크립트와 다를 때만 명시적으로 경고하고 확인받는다.
  ...
  _rb_interrupted() {
    ...
    echo "$(date '+%Y-%m-%d %H:%M:%S')|ROLLBACK|INTERRUPTED|백업=${_selected}|처리=${_idx:-0}/${_total}|로그=${_rb_log}" >> "$FIX_HISTORY_FILE" 2>/dev/null
    echo "INTERRUPTED|처리=${_idx:-0}/${_total}" >> "$_rb_log" 2>/dev/null
    _rb_cleanup
    exit 130
  }
  trap _rb_interrupted INT TERM HUP

  # P4: 최종 확정 직후 실제 파일을 덮어쓰기 전에 현재 상태를 2차 안전 백업한다.
  echo ""
  _div_thin
  ...
  declare -A _rb_baseline_hash=()   # P7: 조치 전 실패 원문 정규화 SHA-256

  # P2: 기본 이력 파일에서 먼저 찾고, 매칭 실패 시 백업 옆 .records를 사용한다.
  ...
  _rb_verify_record() {
    ...
    if [ "$_status" = "FAIL" ] && [ "${_rb_baseline[$_name]:-}" = "FAIL" ]; then
      ...
      # 구버전 레코드(해시 없음)는 기존 동작을 유지하고,
      # 신규 레코드는 FAIL 상태와 정규화 오류 해시가 모두 같을 때만 기준 일치로 인정한다.
  ...
  # (구) 이력 파일 기반 자동 역산 — 파일 비교 전 단계로 이동함
  # 9) 최종 상태 판정
  ...
  # 실제로 남은 차이만 조건부 안내한다. 변경이 없는데도 항상 경고하던 오탐을 제거한다.
...
#   (예전에는 RESULT만 담는 vulnFixExecution_*.log를 별도로 뒀었는데, 호출 지점이
#    항상 상세 로그와 쌍으로 실행되고 내용도 거의 동일해 사실상 중복이었다.
#    상세 로그가 이미 RESULT를 포함한 상위집합이라 실행 로그를 없애고 합쳤다.)
# - 내부 명령·stdout·stderr·종료코드는 상세 로그에만 남긴다.
# - vulnFixHistory.log의 레코드 형식은 유지하고 저장 위치만 report로 이전한다.
# =============================================================================
DETAIL_LOG_FILE=""
if [ "${DRY_RUN:-0}" -eq 0 ]; then
  _EXEC_LOG_DIR="${_RPT_BASE_DIR}/logs"
  if ! mkdir -p "$_EXEC_LOG_DIR" 2>/dev/null || ! chmod 700 "$_EXEC_LOG_DIR" 2>/dev/null; then
    _EXEC_LOG_DIR="${_RPT_BASE_DIR}"
  fi
...
# ANSI CSI 제거 전용 스트림 필터.
# 기존 `[ -\/]` 범위는 일부 sed/로케일 조합에서 "부적절한 대조 문자" 오류를
# 발생시켰다. # 구분자와 `[ -/]` 범위를 사용해 터미널로 오류가 새지 않게 한다.
_strip_ansi_stream() {
  LC_ALL=C sed -E $'s#\x1B\\[[0-?]*[ -/]*[@-~]##g; s#\r##g' 2>/dev/null
}

_log_clean_text() {
  # ANSI/제어문자와 실제·문자열 줄바꿈을 제거해 한 항목을 한 줄로 유지한다.
  printf '%s' "$1" \
    | _strip_ansi_stream \
    | sed ':a;N;$!ba;s/\n/ \/ /g' 2>/dev/null \
...
# "양호" 판정 시 현재 설정값을 BEFORE_VAL(→ CSV/XLSX "조치 전 상태")에 채우는 공용 헬퍼.
# 예전에는 항상 head -3으로 잘라서, world-writable/SUID/중복 UID처럼 결과가
# 여러 줄인 점검은 보고서에 앞 3줄만 보이고 나머지는 vulnFixDetail_*.log를
# 따로 열어야만 보였다(전체는 _detail_log_command로 이미 저장됨). 200줄까지는
# 그대로 담고, 그보다 길면 상세 로그를 보라는 안내만 덧붙인다.
_vf_fill_before_val() {
  local _id="$1" _out="$2" _empty_msg="${3:-이상 항목 없음 (점검 통과)}"
  local _clean; _clean=$(printf '%s' "$_out" | grep -v '^[[:space:]]*$')
  if [ -z "$_clean" ]; then
    BEFORE_VAL["$_id"]="$_empty_msg"
    return 0
  fi
  local _total; _total=$(printf '%s\n' "$_clean" | wc -l | tr -d ' ')
  # 줄바꿈은 실제 개행이 아니라 " || " 토큰으로 잇는다. CSV 저장 시 _csv_esc가
  # 실제 개행을 단일 " | "로 뭉개버리는데, 엑셀 파서(_clean_layout_text)는
  # " || "(이중 파이프)만 개행으로 복원하기 때문 — 실제 개행을 쓰면 200줄짜리
  # 결과가 엑셀에서 한 줄로 뭉쳐 보인다.
  if [ "$_total" -gt 200 ]; then
    BEFORE_VAL["$_id"]=$(printf '%s\n' "$_clean" | head -200 | sed ':a;N;$!ba;s/\n/ || /g')" || ... 외 $((_total-200))줄 더 있음 (전체는 상세 로그 참고: ${DETAIL_LOG_FILE:-미생성})"
  else
    BEFORE_VAL["$_id"]=$(printf '%s\n' "$_clean" | sed ':a;N;$!ba;s/\n/ || /g')
  fi
}

# 점검/검증 명령은 기존과 동일하게 서브셸에서 실행한다.
# 출력·오류·종료코드를 기록하되 판정 기준에는 관여하지 않는다.
...
_detail_log_stage() {
  # _detail_log_stage <항목ID> <CHECK|FIX|VERIFY|RESULT> <설명>
...
_detail_log_command() {
  # _detail_log_command <항목ID> <단계> <명령> <RC> <STDOUT> <STDERR> <상태>
  [ -n "${DETAIL_LOG_FILE:-}" ] || return 0
...
_detail_log_result() {
  # _detail_log_result <항목ID> <표준상태> <메시지>
  [ -n "${DETAIL_LOG_FILE:-}" ] || return 0
...
_detail_log_summary() {
  # _detail_log_summary <양호> <조치완료> <수동확인> <사용자건너뜀> <해당없음> <실패>
  [ -n "${DETAIL_LOG_FILE:-}" ] || return 0
...
# 현재 포트만 닫혔는지가 아니라 systemd 자동기동, xinetd/inetd 설정까지 함께
# 확인하여 재부팅 또는 슈퍼서버 재시작 후 Telnet이 다시 열리는 오탐을 방지한다.
_u52_unit_exists() {
  local _unit="$1"
  command -v systemctl >/dev/null 2>&1 || return 1
...
# ── 실시간 재확인 함수 ────────────────────────────────────────────────────────
# 반환값: 0=취약, 1=양호, 2=해당없음, 3=자동판정 불가(수동확인)
check_still_vuln() {
  local id="$1"
  case "$id" in
    U-01)
    ...
    U-28)
      ...
      # 파일이 "존재"만 해도 양호로 처리되던 오탐 수정 — 실제 규칙 내용이 있어야 양호로 인정
    ...
    U-30)
      # 실제 로그인 시 umask 결정 순서(대부분의 배포판 공통): login.defs(PAM 초기값)
      # → /etc/profile → /etc/bashrc·bash.bashrc → /etc/profile.d/*.sh(보통 profile 맨 끝에서
      # source되어 가장 나중에 적용됨). 기존에는 파일 나열 순서상 "처음 값을 찾은 파일"에서
      # 바로 return해버려 실제 적용 순서와 달라 오탐 여지가 있었다. 이제는 정해진 순서대로
      # 전부 훑어 값을 찾을 때마다 최종값을 덮어쓰고, 마지막에 딱 한 번만 판정한다.
      _u30_final=""
    ...
    U-62)
      ...
      for F in /etc/motd /etc/issue /etc/issue.net; do
        ...
        # 색상 제어 등 무관한 백슬래시까지 전부 취약 처리하던 오탐을 방지한다.
...
# ── 구분선 함수는 스크립트 맨 위(색상 코드 직후)로 이동함 — 여기서는 재정의하지 않음 ──

# ── 사전 백업 ─────────────────────────────────────────────────────────────────
# DRY-RUN 또는 자동 조치 대상이 없는 경우에는 백업 파일을 생성하지 않는다.
if [ "$DRY_RUN" -eq 0 ] && [ "${#_PRECHECK_VULN[@]}" -gt 0 ] && [ "${_NO_ACTION_REQUIRED:-0}" -eq 0 ]; then
_div_thick
...
if [ $_bak_rc -eq 0 ]; then
  ...
  # ── 조치 전 설정 검증 기준값 기록 ─────────────────────────────────────────
  # 롤백 후 검증(sshd -t 등)이 실패해도, 조치 전부터 실패하던 환경이면
  # "복원은 정상인데 검증 실패"로 오탐된다. 조치 전 결과를 기준값으로 남겨
  # 롤백 검증 시 절대 PASS/FAIL이 아니라 기준값과 비교하게 한다.
  _baseline_record() {
    local _k="$1"; shift
    ...
    if "$@" >"$_bl_out" 2>&1; then
      ...
      if [ -n "$_bl_hash" ]; then
        ...
        # sha256sum이 없는 환경은 구버전 형식으로 기록해 기존 판정과 호환한다.
...
# ── 공통 설정값 수정 엔진 (0단계: 함수 정의만 추가, 아직 호출부 없음) ────────────
# config_set <file> <key> <value> [mode] [sep] [ci]
#   mode: kv      → "key = value" 계열 (sep로 구분자 지정, 기본 " = ")
#         kv_tab  → "key<TAB>value" (login.defs류)
#         line    → key를 정규식(ERE)으로 매치, 매치되면 라인 전체를 value로 치환.
#                   매치 안 되면 파일 끝에 value 추가
#         substr  → key에 매치된 "부분 문자열"만 value로 치환 (줄의 나머지는 보존).
#                   백레퍼런스(\1 등) 사용 가능. value가 빈 문자열이면 해당 부분
#                   삭제(제거) 용도로도 쓴다. 매치 자체가 없으면 이미 양호로 보고
#                   무변경(2) 반환 — line 모드와 달리 파일 끝에 추가하지 않는다.
#         delete  → key에 매치되는 "줄 전체"를 삭제. 매치 없으면 무변경(2).
#   ci  : "ci"를 넘기면 line/substr/delete 모드에서 대소문자 무시 매치/치환
#         (sshd_config 등 지시어가 대소문자를 가리지 않는 파일용 — 기존 sed 's/../I' 대체)
# 반환값: 0=변경발생, 1=실패(파일없음 등), 2=변경없음(이미 원하는 값)
#
# 주의: PAM 삽입, exports IP 치환 등 여러 값이 얽힌 조건부 삽입 로직은
#       이 함수로 통일하지 않고 기존 로직을 유지한다.
config_set() {
  local file="$1"
  local key="$2"
  local value="$3"
...
# ── UI 레이블 헬퍼(_ok/_fail/_info/_warn)는 스크립트 맨 위로 이동함 — 재정의하지 않음 ──
...
# ── 섹션 헤더(_sec), 테이블 행(_row)은 스크립트 맨 위로 이동함 — 재정의하지 않음 ──
...
_result_persist_allowed() { [ "${DRY_RUN:-0}" -eq 0 ]; }
_mark_fixed() {
  FIXED=$((FIXED+1)); FIXED_LIST+=("$1: $2")
  _result_persist_allowed || return 0
...
_mark_dryrun() { DRYRUN=$((DRYRUN+1)); DRYRUN_LIST+=("$1: $2"); }

# config_set 반환값(0=변경, 2=무변경, 기타=실패)을 사람이 읽기 쉽게 출력
_cs_report() {
...
# ── DRY-RUN 공통 출력 (변경 계획서) ───────────────────────────────────────────
# 모든 U-01~U-76 항목이 DRY-RUN에서 동일한 레이아웃을 쓰도록 통일한 렌더러.
# fix_cmd(쉘 코드) 원문은 절대 화면에 출력하지 않는다 — 오직 "무엇을 할 예정인지"
# 만 사람이 읽는 문장으로 보여준다. 판정/조치/검증/롤백/백업/보고서 로직은 이
# 함수와 무관하며 그대로 유지된다.
# _dryrun_plan <id> <title> <현재값> <변경예정값> <변경파일목록> [예정 명령 요약]
_dryrun_plan() {
  local id="$1" title="$2" before_val="$3" change_val="$4" files="$5" ops="${6:-}"

  # CSV/엑셀용 값 자동 세팅 — _mark_dryrun 호출 시점에 이미 세팅돼 있어야 함
  [ -z "${BEFORE_VAL[$id]:-}" ] && BEFORE_VAL["$id"]="${before_val:-설정 정보 없음 (점검 대상 미감지)}"
  AFTER_VAL["$id"]="DRY-RUN (미실행)"
  local _dp=""
  [ -n "$before_val" ]  && _dp="[현재 설정] ${before_val}"
  [ -n "$change_val" ]  && _dp="${_dp:+$_dp | }[변경 예정] ${change_val}"
  [ -n "$files" ]       && _dp="${_dp:+$_dp | }[변경 예정 파일] $(echo "$files" | tr '\n' ' ' | sed 's/  */ /g; s/ $//')"
  [ -z "${DETAIL_VAL[$id]:-}" ] && DETAIL_VAL["$id"]="${_dp:-[DRY-RUN] 변경 예정 없음}"

  echo -e "${YELLOW}[DRY-RUN]${RESET}"
  echo -e "${BOLD}${id}${RESET} ${title}"
  echo ""
  echo -e "${BOLD}현재 설정${RESET}"
  _div_sec
  if [ -n "$before_val" ]; then
    echo "$before_val" | sed 's/^/   /'
  else
    echo "   (설정값 없음 — 미설정 상태)"
  fi
  echo ""
  echo -e "${BOLD}변경 예정${RESET}"
  _div_sec
  if [ -n "$change_val" ]; then
    echo "$change_val" | sed 's/^/   /'
  else
    echo "   보안 권장 설정으로 조정 예정"
  fi
  echo ""
  echo -e "${BOLD}변경 파일${RESET}"
  _div_sec
  if [ -n "$files" ]; then
    echo "$files" | sed '/^[[:space:]]*$/d; s/^/   /'
  else
    echo "   (해당 파일 없음)"
  fi
  echo ""
  echo -e "${BOLD}예정 작업${RESET}"
  _div_sec
  if [ -n "$ops" ] && { echo "$ops" | grep -qE 'cp[[:space:]]+-p|_backup|backup|/etc/'; }; then
    echo -e "   ${GREEN}✓${RESET} 설정 파일 백업 예정"
  fi
  if [ -n "$ops" ] && echo "$ops" | grep -qE 'yum|dnf|apt(-get)?|zypper'; then
    echo -e "   ${GREEN}✓${RESET} 패키지 설치/업데이트 예정"
  elif [ -n "$ops" ] && echo "$ops" | grep -qE 'chmod|chown'; then
    echo -e "   ${GREEN}✓${RESET} 권한/소유권 변경 예정"
  else
    echo -e "   ${GREEN}✓${RESET} 설정 변경 예정"
  fi
  if [ -n "$ops" ] && echo "$ops" | grep -qE 'systemctl[[:space:]]+(stop|disable|mask)'; then
    echo -e "   ${GREEN}✓${RESET} 서비스 중지/비활성화 예정"
  fi
  echo -e "   ${GREEN}✓${RESET} 변경 결과 검증 예정"
  if [ -n "$ops" ] && echo "$ops" | grep -qE 'systemctl[[:space:]]+(restart|reload|try-restart|reload-or-restart)'; then
    echo -e "   ${GREEN}✓${RESET} 서비스 재시작/reload 예정"
  fi
  echo ""
  echo -e "   ${CYAN}※ 실제 시스템은 변경되지 않았습니다.${RESET}"
  echo ""
}

# _dryrun_change_summary <fix_cmd>
# fix_cmd 문자열에서 config_set(공통 수정 엔진) 호출의 목표 값(3번째 인자)만
# 추출해 "변경 예정" 섹션에 쓸 사람이 읽는 값 목록을 만든다. config_set을 쓰지
# 않는 커스텀 로직(PAM 삽입 등)은 값 추출이 안 되므로 빈 문자열을 반환하고,
# 이 경우 _dryrun_plan이 알아서 기본 문구로 대체한다.
_dryrun_change_summary() {
  local fix_cmd="$1"
  grep -oP "config_set\s+\S+\s+'[^']*'\s+'\K[^']*" <<< "$fix_cmd" 2>/dev/null \
    | grep -v '^[[:space:]]*$' | sort -u
}
...
do_fix() {
  ...
  # ── 자동판정 불가: 수동확인 ────────────────────────────────────────────────
    ...
    BEFORE_VAL["$id"]="${_manual_out:-버전 정보 확인 필요}"
    AFTER_VAL["$id"]="수동 확인 필요"
    DETAIL_VAL["$id"]="[현재 상태] ${_manual_out:-버전 정보 확인 필요} | [판정] 저장소/벤더 정보 확인 불가로 수동확인"
  ...
  local _before_trim; _before_trim=$(echo "$before_out" | grep -v '^[[:space:]]*$' | head -5)

  # DRY-RUN: 실제로 아무것도 하지 않으므로 조치 여부를 묻지 않고 바로
  # 통일된 "변경 계획서" 카드만 보여준다 (76개 항목 전체 동일 레이아웃).
  if [ "$DRY_RUN" -eq 1 ]; then
    _item_header "vuln" "$id" "$title"
    local _dr_files; _dr_files=$(grep -oE '/(etc|var|usr/local|opt|root|home)/[A-Za-z0-9_@+.,/:-]+' <<< "$fix_cmd" | sed 's/[;,]$//' | sort -u)
    local _dr_change; _dr_change=$(_dryrun_change_summary "$fix_cmd")
    # _dryrun_plan 내부에서 BEFORE_VAL/AFTER_VAL/DETAIL_VAL 자동 세팅
    _dryrun_plan "$id" "$title" "$_before_trim" "$_dr_change" "$_dr_files" "$fix_cmd"
    # _before_trim이 비어있어도 _dryrun_plan이 채웠을 수 있으므로 덮어쓰지 않음
    [ -n "$_before_trim" ] && BEFORE_VAL["$id"]="$_before_trim"
    _mark_dryrun "$id" "$title"
    return
  fi
...
do_manual() {
  ...
  if [ $vuln_status -eq 1 ]; then
    ...
    _item_close na
    BEFORE_VAL["$id"]="${_manual_before:-점검값 없음 (수동 확인 필요)}"
    ...
    DETAIL_VAL["$id"]="[현재 상태] ${_manual_before:-(점검값 없음)} | [조치 방법] ${_desc_report}"
...
# U-01 root 계정 원격 접속 제한 — SSH + Telnet(PAM) 분기 처리
{
  ...
  if [ $_match01 -eq 1 ]; then                                  # (1) TARGET_IDS 매칭
    ...
    if [ $_vs -eq 1 ]; then                                     # (2) 이미 양호?
      ...
      # ── DRY-RUN ───────────────────────────────────────────────────────────
      if [ "$DRY_RUN" -eq 1 ]; then
        _u01_dr_before="PermitRootLogin: ${_u01_literal:-미설정 (기본값 yes)}"
        _u01_dr_change="PermitRootLogin no 로 설정"
        if [ "$_u01_telnet_on" -eq 1 ]; then
          _u01_dr_before="${_u01_dr_before}
Telnet: 활성"
          _u01_dr_change="${_u01_dr_change}
Telnet 사용 중 — pam_securetty.so 연결, securetty pts/ 주석 처리"
        else
          _u01_dr_before="${_u01_dr_before}
Telnet: 미사용"
        fi
        _dryrun_plan "U-01" "(상) root 계정 원격 접속 제한" "$_u01_dr_before" "$_u01_dr_change" \
          "$(printf '%s\n' "/etc/ssh/sshd_config" "/etc/ssh/sshd_config.d/*.conf (해당 시)" "/etc/pam.d/login (Telnet 활성 시)" "/etc/securetty (Telnet 활성 시)")"
        _mark_dryrun "U-01" "root 계정 원격 접속 제한"
      else
      # ── /DRY-RUN ──────────────────────────────────────────────────────────
      _lbl_yn
...
# U-02 비밀번호 관리정책 — PASS_MAX_DAYS 사용자 선택 입력
{
  ...
  _flush_header
    if [ $_vs -eq 1 ]; then
      ...
      # ── DRY-RUN ───────────────────────────────────────────────────────────
      if [ "$DRY_RUN" -eq 1 ]; then
        _u02_dr_before=$(grep -v '^\s*#' /etc/login.defs 2>/dev/null | grep -E 'PASS_MAX_DAYS|PASS_MIN_DAYS'; \
          if [ -n "$_u02_minlen_out" ]; then echo "$_u02_minlen_out"; else echo "minlen 미설정"; fi)
        _u02_dr_change="PASS_MAX_DAYS=${DEFAULT_PASS_MAX_DAYS}, PASS_MIN_DAYS=${DEFAULT_PASS_MIN_DAYS}
minlen=${DEFAULT_MINLEN}, ucredit/lcredit/dcredit/ocredit=-1"
        _dryrun_plan "U-02" "(상) 비밀번호 관리정책 설정" "$_u02_dr_before" "$_u02_dr_change" \
          "$(printf '%s\n' "/etc/login.defs" "/etc/security/pwquality.conf" "/etc/pam.d/system-auth, password-auth 등 (PAM 적용 선택 시)")"
        _mark_dryrun "U-02" "비밀번호 관리정책"
      else
      # ── /DRY-RUN ──────────────────────────────────────────────────────────
      _lbl_yn
      if [ "$NO_PROMPT" -eq 1 ]; then
      ...
      if [ "$PAM_APPLY" -eq 1 ] && command -v authselect &>/dev/null && authselect current &>/dev/null; then
        ...
        # 프로필 변경 등) 조용히 초기화(원상복구)될 수 있다.
        # 현재 authselect profile 정보를 이력 파일에 저장 (복구 참고용)
        {
...

{
  ...
  _flush_header
    if [ $_vs -eq 1 ]; then
      ...
      # ── DRY-RUN ───────────────────────────────────────────────────────────
      if [ "$DRY_RUN" -eq 1 ]; then
        _u03_dr_before=$(for _pf in /etc/security/faillock.conf /etc/pam.d/system-auth /etc/pam.d/password-auth /etc/pam.d/common-auth; do
          [ -f "$_pf" ] && grep -v '^#' "$_pf" | grep -E 'deny|unlock_time|pam_tally|pam_faillock' | sed "s|^|[$_pf] |" | head -3
        done)
        _u03_dr_change="deny=${DEFAULT_DENY}, unlock_time=${DEFAULT_UNLOCK_TIME}
pam_faillock.so preauth/authfail 라인 연결"
        _dryrun_plan "U-03" "(상) 계정 잠금 임계값 설정" "$_u03_dr_before" "$_u03_dr_change" \
          "$(printf '%s\n' "/etc/security/faillock.conf" "/etc/pam.d/system-auth" "/etc/pam.d/password-auth")"
        _mark_dryrun "U-03" "계정 잠금 임계값"
      else
      # ── /DRY-RUN ──────────────────────────────────────────────────────────
      _lbl_yn
      if [ "$NO_PROMPT" -eq 1 ]; then
...

{
  ...
  if [ $_match -eq 1 ]; then
    ...
    if [ $_vs -eq 1 ]; then
      ...
      # ── DRY-RUN ───────────────────────────────────────────────────────────
      if [ "$DRY_RUN" -eq 1 ]; then
        _dryrun_plan "U-06" "(상) 사용자 계정 su 기능 제한" "${_u06_wheel_out:-pam_wheel 미설정}" \
          "pam_wheel.so use_uid group=wheel 옵션 추가
wheel 그룹에 로그인 가능한 일반 계정 추가 (운영자 선택)" \
          "$(printf '%s\n' "/etc/pam.d/su" "/etc/group (wheel 멤버, 해당 시)")"
        _mark_dryrun "U-06" "su 기능 제한 (pam_wheel)"
      else
      # ── /DRY-RUN ──────────────────────────────────────────────────────────
      _lbl_yn
...

{
  ...
  if [ $_match -eq 1 ]; then
    ...
    # ── [취약] ──────────────────────────────────────────────────────────────
      ...
      # ── DRY-RUN ───────────────────────────────────────────────────────────
      if [ "$DRY_RUN" -eq 1 ]; then
        _u12_dr_before="${_common_out:-TMOUT 미설정}"
        [ -n "$_bypass_out" ] && _u12_dr_before="${_u12_dr_before}
우회 설정 탐지됨"
        _dryrun_plan "U-12" "(하) 세션 종료 시간 설정" "$_u12_dr_before" \
          "TMOUT=${DEFAULT_TMOUT} (export, readonly)
기존 우회 설정(TMOUT=0 등) 주석 처리" \
          "$(printf '%s\n' "/etc/profile (TMOUT 직접 추가/수정)" "/etc/profile.d/*.sh, /etc/bashrc (기존 TMOUT 중복 주석 처리)")"
        _mark_dryrun "U-12" "세션 타임아웃 (TMOUT=${DEFAULT_TMOUT})"
      else
      # ── /DRY-RUN ──────────────────────────────────────────────────────────
      _lbl_yn
      if [ "$NO_PROMPT" -eq 1 ]; then
      ...
      if [[ "$_yn_u12" != [Yy] ]]; then
        ...
        # TMOUT 설정을 /etc/profile 에 직접 추가/수정한다.
        # 신규 파일 생성 방식은 /etc/profile.d 로드 순서에 따라 이후 파일이
        # 덮어쓸 수 있고 롤백 시 파일 삭제가 필요한 문제가 있어,
        # /etc/profile 에 직접 기재하는 방식으로 통일한다.
  ...
  'for f in /etc/profile /etc/bashrc /etc/bash.bashrc /root/.bash_profile /root/.bashrc /root/.profile; do
     ...
     while IFS= read -r _u14_line || [ -n "$_u14_line" ]; do
       if echo "$_u14_line" | grep -qE "^[[:space:]]*(export[[:space:]]+)?PATH="; then
         ...
         # ':.:' , 맨앞 '.:' , 맨뒤 ':.' 등 위치와 무관하게 "." 세그먼트만 제거하고
         # 나머지 PATH 항목은 그대로 보존한다 (과거엔 줄 전체를 삭제하는 버그가 있었음).
...
# U-25 World Writable
{
  ...
  if [ $_match -eq 1 ]; then
    ...
    if [ $_vs -eq 1 ]; then
      ...
      if [ "$DRY_RUN" -eq 1 ]; then
        _u25_dr_before="미승인 world writable 일반 파일 ${_u25_target_cnt}개"
        [ "$_u25_approved_cnt" -gt 0 ] && _u25_dr_before="${_u25_dr_before}\n설정 사유 확인 파일 ${_u25_approved_cnt}개"
        _dryrun_plan "U-25" "(상) world writable 파일 점검" "$_u25_dr_before" \
          "파일별 선택: 1) other 쓰기 권한 제거  2) 설정 사유 확인 후 유지  3) 변경하지 않음" \
          "$(printf '%s\n' "$_u25_targets" | sed '/^$/d' | head -10)"
        AFTER_VAL["U-25"]="DRY-RUN (미실행)"
        _mark_dryrun "U-25" "(상) world writable 파일 점검"
      else
        if [ "${NO_PROMPT:-0}" -eq 1 ]; then
          _yn25="y"
...

{
  ...
  if [ $_match -eq 1 ]; then
    ...
    if [ $_vs -eq 1 ]; then
      ...
      if [ "$DRY_RUN" -eq 1 ]; then
        _u28_dr_ports=$(sshd -T 2>/dev/null | grep -i '^port ' | awk '{print $2}' | sort -u)
        [ -z "$_u28_dr_ports" ] && _u28_dr_ports="22"
        _dryrun_plan "U-28" "(상) 접속 IP 및 포트 제한" \
          "firewalld: $(systemctl is-active firewalld 2>/dev/null)
iptables 룰: ${_u28_ipt}개" \
          "감지된 SSH 포트(${_u28_dr_ports//$'\n'/,}) 허용 + 나머지 인바운드 차단
firewalld/ufw/iptables 중 사용 가능한 것으로 적용" \
          "$(printf '%s\n' "/etc/hosts.allow" "/etc/hosts.deny" "firewalld/ufw/iptables 규칙 (환경에 따라 다름)")"
        _mark_dryrun "U-28" "(상) 접속 IP 및 포트 제한"
      else
      _lbl_yn
...
do_fix "U-30" "(중) UMASK 설정 관리" \
  ...
  '# 0) umask 값이 KISA 022 이상(비트마스크 기준 — 그룹/기타 쓰기권한 제거)인지 판정.
   ...
   # 1) 각 설정 파일에서 취약한 umask 줄만 제거 (이미 022 이상인 줄은 보존 — 과거엔
   #    sed 대체 패턴이 -E 없이 (022|027)를 리터럴로 취급해 이미 올바른 줄까지 삭제되었음)
...
# U-39 NFS (업무 여부 확인 필요)
{
  ...
  _flush_header
    if [ $_vs -eq 1 ]; then
      ...
      if [ "$DRY_RUN" -eq 1 ]; then
        _dryrun_plan "U-39" "(상) 불필요한 NFS 서비스 비활성화" \
          "nfs-server: $(systemctl is-active nfs-server 2>/dev/null)
$(mount | grep nfs 2>/dev/null)" \
          "업무상 불필요로 확인될 경우 nfs-server/nfs-mountd/rpc-statd 중지·비활성화·마스킹" \
          "(설정 파일 변경 없음 — 서비스 상태만 변경)"
        AFTER_VAL["U-39"]="DRY-RUN (미실행)"
        _mark_dryrun "U-39" "(상) 불필요한 NFS 서비스 비활성화"
        _NFS_DISABLED=0
      else
  ...
  if [ $_match -eq 1 ]; then
    ...
    if [ $_vs -eq 1 ]; then
      ...
      if [ "$DRY_RUN" -eq 1 ]; then
        _u35_dr_change=""
        for _acc in ftp anonymous; do
          grep -q "^${_acc}:" /etc/passwd 2>/dev/null && _u35_dr_change="${_u35_dr_change}${_acc} 계정 잠금(usermod -L) + 셸을 /sbin/nologin으로 변경
"
        done
        for _cf in /etc/vsftpd.conf /etc/vsftpd/vsftpd.conf; do
          [ -f "$_cf" ] && _u35_dr_change="${_u35_dr_change}${_cf}: anonymous_enable=NO 설정
"
        done
        for _cf in /etc/proftpd.conf /etc/proftpd/proftpd.conf; do
          [ -f "$_cf" ] && grep -qiE '^\s*<Anonymous' "$_cf" 2>/dev/null && _u35_dr_change="${_u35_dr_change}${_cf}: <Anonymous> 블록 주석 처리
"
        done
        if [ -f /etc/exports ]; then
          grep -v '^#' /etc/exports 2>/dev/null | grep -qE '^\s*/.*\*\s*\(' && _u35_dr_change="${_u35_dr_change}/etc/exports: 전체 허용(*) → 신뢰 IP로 제한 (운영자 입력 필요)
"
          grep -v '^#' /etc/exports 2>/dev/null | grep -q 'no_root_squash' && _u35_dr_change="${_u35_dr_change}/etc/exports: no_root_squash 제거 (root_squash 적용)
"
        fi
        [ -z "$_u35_dr_change" ] && _u35_dr_change="익명 접근 차단 설정 적용"
        _dryrun_plan "U-35" "(상) 공유 서비스에 대한 익명 접근 제한 설정" "$_u35_exports_pre" "$_u35_dr_change" \
          "$(printf '%s\n' "/etc/vsftpd.conf" "/etc/proftpd.conf" "/etc/exports" "/etc/passwd (ftp/anonymous 계정)")"
        _mark_dryrun "U-35" "(상) 공유 서비스에 대한 익명 접근 제한 설정"
      else
      _lbl_yn
  ...
  if [ $_match -eq 1 ]; then
    ...
    if [ $_vs -eq 1 ]; then
      ...
      if [ "$DRY_RUN" -eq 1 ]; then
        ...
        for _bin37 in "$_u37_crontab_bin" "$_u37_at_bin"; do
          [ -f "$_bin37" ] || continue
          _u37_dr_current="${_u37_dr_current}$(stat -c "${_bin37} : %U / %a" "$_bin37" 2>/dev/null)\n"
          _u37_dr_files="${_u37_dr_files}${_bin37}\n"
        done
        for F in /etc/crontab /etc/cron.allow /etc/cron.deny /etc/at.allow /etc/at.deny; do
          [ -f "$F" ] || continue
          _u37_dr_current="${_u37_dr_current}$(stat -c "${F} : %U / %a" "$F" 2>/dev/null)\n"
          _u37_dr_files="${_u37_dr_files}${F}\n"
        done
        for D in /etc/cron.d /etc/cron.daily /etc/cron.hourly /etc/cron.monthly /etc/cron.weekly \
                 /var/spool/cron /var/spool/cron/crontabs /var/spool/at /var/spool/atjobs; do
          [ -d "$D" ] || continue
          _u37_dr_current="${_u37_dr_current}$(stat -c "${D} : %U / %a" "$D" 2>/dev/null)\n"
          _u37_dr_files="${_u37_dr_files}${D}\n"
        done
        for D in /etc/cron.d /var/spool/cron /var/spool/cron/crontabs /var/spool/at /var/spool/atjobs; do
          [ -d "$D" ] || continue
          while IFS= read -r -d '' F; do
        ...
        _dryrun_plan "U-37" "(상) crontab 설정파일 권한 설정 미흡" \
          "$(printf '%b' "$_u37_dr_current")" \
          "crontab/at 명령 파일: root:root / 750 (SUID/SGID 제거)
cron/at 설정 및 작업 목록 파일: root:root / 640
cron/at 관련 디렉터리: root:root / 750" \
          "$(printf '%b' "$_u37_dr_files")"
        AFTER_VAL["U-37"]="DRY-RUN (미실행)"
        _mark_dryrun "U-37" "(상) crontab 설정파일 권한 설정 미흡"
      else
        _lbl_yn
  ...
  if [ $_match -eq 1 ]; then
    ...
    if [ $_vs -eq 1 ]; then
      ...
      if [[ "$_u54_inuse" == [Yy] ]]; then
        ...
        if [ "$DRY_RUN" -eq 1 ]; then
          _dryrun_plan "U-54" "(중) 암호화되지 않는 FTP 서비스 비활성화" \
            "서비스: ${_u54_svc:-확인불가} (운용 중)" \
            "vsftpd/proftpd 서비스 중지 및 비활성화" \
            "(설정 파일 변경 없음 — 서비스 상태만 변경)"
          AFTER_VAL["U-54"]="DRY-RUN (미실행)"
          _mark_dryrun "U-54" "(중) 암호화되지 않는 FTP 서비스 비활성화"
        else
        _lbl_yn
  ...
  if [ $_match -eq 1 ]; then
    ...
    if [ $_vs -eq 1 ]; then
      ...
      if [[ "$_u58_inuse" == [Yy] ]]; then
        ...
        if [ "$DRY_RUN" -eq 1 ]; then
          _dryrun_plan "U-58" "(중) 불필요한 SNMP 서비스 구동 점검" \
            "상태: $(systemctl is-active snmpd 2>/dev/null) (운용 중)" \
            "snmpd 서비스 중지, 비활성화, 마스킹" \
            "(설정 파일 변경 없음 — 서비스 상태만 변경)"
          AFTER_VAL["U-58"]="DRY-RUN (미실행)"
          _mark_dryrun "U-58" "(중) 불필요한 SNMP 서비스 구동 점검"
        else
        _lbl_yn
...

{
  ...
  _flush_header
    if [ $_vs -eq 1 ]; then
      ...
      if [ "$DRY_RUN" -eq 1 ]; then
        _dryrun_plan "U-62" "(하) 로그인 시 경고 메시지 설정" \
          "/etc/issue: ${_u62_issue:-없음}
/etc/issue.net: ${_u62_issuenet:-없음}
sshd Banner: ${_u62_sshbanner:-미설정}" \
          "/etc/issue, /etc/issue.net, /etc/motd에 경고 배너 문구 적용
sshd_config Banner /etc/issue.net 설정 후 reload" \
          "$(printf '%s\n' "/etc/issue" "/etc/issue.net" "/etc/motd" "/etc/ssh/sshd_config")"
        AFTER_VAL["U-62"]="DRY-RUN (미실행)"
        _mark_dryrun "U-62" "(하) 로그인 시 경고 메시지 설정"
      else
      _lbl_yn
...
# U-64 보안 패치 (패키지 관리자 및 구독 등록 여부에 따라 분기)
{
  ...
  _flush_header
    if [ $_vs -eq 1 ]; then
      ...
      if [ "$DRY_RUN" -eq 1 ]; then
        _u64_dr_change="apt-get upgrade -y (전체 업그레이드, ${CNT}개)"
        [ -n "$_u64_sec_pkgs" ] && _u64_dr_change="apt-get install --only-upgrade -y (보안 패키지 $(echo "$_u64_sec_pkgs" | wc -l)개)"
        _dryrun_plan "U-64" "(상) 주기적 보안 패치 및 벤더 권고사항 적용" \
          "업그레이드 가능 패키지: ${CNT}개" \
          "$_u64_dr_change" \
          "$(echo "${_u64_sec_pkgs:-$(apt list --upgradable 2>/dev/null | grep '\[upgradable' | cut -d/ -f1)}" | head -10)"
        AFTER_VAL["U-64"]="DRY-RUN (미실행)"
        _mark_dryrun "U-64" "(상) 주기적 보안 패치 및 벤더 권고사항 적용"
      else
      ...
      if [ "$DRY_RUN" -eq 1 ]; then
        _dryrun_plan "U-64" "(상) 주기적 보안 패치 및 벤더 권고사항 적용" \
          "적용 가능한 보안 패치: ${SEC_COUNT}개" \
          "yum update --security -y (대상 ${SEC_COUNT}개)" \
          "(패키지 업데이트 — 개별 설정 파일 변경 아님)"
        AFTER_VAL["U-64"]="DRY-RUN (미실행)"
        _mark_dryrun "U-64" "(상) 주기적 보안 패치 및 벤더 권고사항 적용"
      else
...
# 아무 일도 하지 않는 무해한 동작).
if [ "$DRY_RUN" -eq 0 ] && command -v getenforce &>/dev/null \
...
[ "$DRYRUN" -gt 0 ] && \
echo -e " ${YELLOW}◌ DRY-RUN (미실행)${RESET} : ${BOLD}${YELLOW}${DRYRUN}건${RESET}"
...
# DRY-RUN은 화면 확인 전용이므로 여기서 종료한다.
# 실제 설정, 백업, 이력 로그, TXT/CSV/XLSX 파일은 생성하지 않는다.
if [ "$DRY_RUN" -eq 1 ]; then
  echo ""
  echo -e " ${GREEN}✔ Dry Run 완료${RESET}"
  echo ""
  _row "실제 변경 파일" "0개"
  _row "백업 생성" "없음"
  _row "보고서 생성" "없음"
  echo ""
  _info "이 모드는 변경 예정 사항만 확인하는 모의 실행입니다."
  _info "실제 적용: ./$(basename "$0")"
  echo ""
  exit 0
fi
...
# =============================================================================
# [변경 금지] 요약 대시보드 UI 보호 규칙
# - "요약 대시보드" 시트의 배치, 차트, 범주, 범례, 제목, 색상 및 크기는
#   사용자의 명시적인 수정 요청이 없는 한 절대 변경하지 않는다.
# - 롤백, 취약점 조치, 상세 시트 및 기타 기능 수정 시에도 이 영역은 그대로 보존한다.
# =============================================================================
...
risk_data = [['위험도','양호','취약'], ['상', RISK_STAT['상'][0], RISK_STAT['상'][1]], ['중', RISK_STAT['중'][0], RISK_STAT['중'][1]], ['하', RISK_STAT['하'][0], RISK_STAT['하'][1]]]
for r, row in enumerate(risk_data, 15):
    for c, v in enumerate(row, 6): cell(ws, r, c, v)
style_table(ws, 15, 6, 18, 8)

cat_data = [['항목 분류','점수(%)','상','중','하']] + [[x[0], x[1], x[2], x[3], x[4]] for x in CAT_STAT]
...
# Excel 범례 순서를 참조 이미지와 동일하게 '취약'(빨강) → '양호'(파랑)로 고정한다.
_bar2_categories = f"'{ws.title}'!$F$16:$F$18"
_bar2_bad = Series(Reference(ws, min_col=8, min_row=16, max_row=18), title='취약')
...
_bar2_good = Series(Reference(ws, min_col=7, min_row=16, max_row=18), title='양호')
...
header(ws, 23, 10, 13, '5. 항목 분류별 보안 점수')
...
bar3.title = '항목 분류별 보안 점수(%)'
bar3.y_axis.title = '점수(%)'
...
_bar3_series = Series(Reference(ws, min_col=11, min_row=16, max_row=_cat_end_row), title='점수(%)')
...
# ── 백업·로그 ────────────────────────────────────────────────────────────────
ws4 = wb.create_sheet('백업·로그')
ws4.sheet_view.showGridLines = False
header(ws4, 2, 2, 4, '실행 정보')
info = [('서버명', server), ('OS 정보', os_info), ('실행 일시', run_ts), ('결과 CSV', csv_path), ('이력 로그', history_path)]
for i, (k, v) in enumerate(info, 3):
    cell(ws4, i, 2, k, font=FONT_BOLD, fill=PALE, align='left')
    ws4.merge_cells(start_row=i, start_column=3, end_row=i, end_column=4)
    cell(ws4, i, 3, v, font=FONT_BASE, fill=WHITE, align='left', wrap=True)
header(ws4, 10, 2, 4, '항목별 백업 경로')
for c, v in enumerate(['항목ID','항목명','백업 경로'], 2): cell(ws4, 11, c, v, font=FONT_HEADER, fill=FILL_NAVY)
_backup_rows = [(x, format_backup_path(x)) for x in rows]
_backup_rows = [(x, path) for x, path in _backup_rows if path]
if _backup_rows:
    for i, (x, path) in enumerate(_backup_rows, 12):
        for c, v in enumerate([x.get('항목ID',''), x.get('항목명',''), path], 2):
            cell(ws4, i, c, v, font=FONT_SMALL, align='left', wrap=True)
else:
    ws4.merge_cells(start_row=12, start_column=2, end_row=12, end_column=4)
    cell(ws4, 12, 2, '기록된 백업 경로 없음', font=FONT_SMALL, fill=WHITE, align='left')
set_widths(ws4, [2,10,34,70])
...
# P2: 조치가 끝난 시점의 역산 레코드를 백업 옆에 독립 보관한다.
# 다른 서버로 백업을 이동할 때 tar.gz와 .records를 함께 복사하면
# vulnFixHistory.log가 없거나 해당 실행 블록을 찾지 못해도 추가 복구가 가능하다.
