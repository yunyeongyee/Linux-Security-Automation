...
# U-02 비밀번호 관리정책 — PASS_MAX_DAYS 사용자 선택 입력
{
  ...
  _flush_header
    if [ $_vs -eq 1 ]; then
      ...
      if [[ "$_max_input" =~ ^[0-9]+$ ]] && [ "$_max_input" -ge 1 ] && [ "$_max_input" -le 365 ]; then
        ...
        echo -e " ${YELLOW}잘못된 입력입니다. 기본값 ${DEFAULT_PASS_MAX_DAYS}일로 설정합니다.${RESET}"
      ...
      read -rp " 최소 사용기간(일) 입력: " _min_input
      if [[ "$_min_input" =~ ^[0-9]+$ ]] && [ "$_min_input" -ge 1 ] 2>/dev/null; then
        MIN_DAYS=$_min_input
      else
        echo -e " ${YELLOW}잘못된 입력입니다. 기본값 ${DEFAULT_PASS_MIN_DAYS}일로 설정합니다.${RESET}"
      ...
      read -rp "     입력: " _len_input
      if [[ "$_len_input" =~ ^[0-9]+$ ]] && [ "$_len_input" -ge 8 ] 2>/dev/null; then
        MINLEN=$_len_input
      else
        echo -e " ${YELLOW}잘못된 입력입니다. 기본값 ${DEFAULT_MINLEN}자로 설정합니다.${RESET}"
      ...
      if [[ "$_u02_credit_default" == [Yy] ]]; then
        ...
        for _pair in "ucredit:대문자:UCREDIT" "lcredit:소문자:LCREDIT" "dcredit:숫자:DCREDIT" "ocredit:특수문자:OCREDIT"; do
          ...
          read -rp "     입력: " _cr_input
          if [[ "$_cr_input" =~ ^[0-9]+$ ]] 2>/dev/null; then
            printf -v "$_varname" '%d' "$(( -_cr_input ))"
          else
            echo -e " ${YELLOW}잘못된 입력. 기본값 1개로 설정합니다.${RESET}"
            printf -v "$_varname" '%d' -1
          fi
        done
      fi

      echo -e "     비밀번호 재시도 횟수(retry, 권고: 3)"
      read -rp "     retry 입력: " _retry_input
      if [[ "$_retry_input" =~ ^[0-9]+$ ]] && [ "$_retry_input" -ge 1 ] 2>/dev/null; then
        RETRY=$_retry_input
      else
        echo -e " ${YELLOW}잘못된 입력입니다. 기본값 3으로 설정합니다.${RESET}"
        RETRY=3
      fi
      ...
      if [ "$PAM_APPLY" -eq 1 ]; then
        for pamf in /etc/pam.d/system-auth /etc/pam.d/password-auth; do
          [ -f "$pamf" ] || continue
          if grep -q 'pam_pwquality.so' "$pamf"; then
            sed -i "s|.*pam_pwquality.so.*|password required pam_pwquality.so enforce_for_root|" "$pamf"
          else
...

{
  ...
  _flush_header
    if [ $_vs -eq 1 ]; then
      ...
      read -rp " 실패 횟수 입력: " _deny_input
      if [[ "$_deny_input" =~ ^[0-9]+$ ]] && [ "$_deny_input" -ge 1 ] && [ "$_deny_input" -le 10 ] 2>/dev/null; then
        DENY_VAL=$_deny_input
      else
        echo -e " ${YELLOW}잘못된 입력. 기본값 ${DEFAULT_DENY}회로 설정합니다.${RESET}"
      ...
      read -rp " 잠금 해제 시간(초) 입력: " _unlock_input
      if [[ "$_unlock_input" =~ ^[0-9]+$ ]] && [ "$_unlock_input" -ge 1 ] 2>/dev/null; then
        UNLOCK_VAL=$_unlock_input
      else
        echo -e " ${YELLOW}잘못된 입력. 기본값 ${DEFAULT_UNLOCK_TIME}초로 설정합니다.${RESET}"
  ...
  if [ $_match -eq 1 ]; then
    ...
    # ── [취약] ──────────────────────────────────────────────────────────────
      ...
      if [[ "$_yn_u12" != [Yy] ]]; then
        ...
        if [[ "$_tmout_input" =~ ^[0-9]+$ ]] && \
           [ "$_tmout_input" -ge 1 ] && [ "$_tmout_input" -le 600 ] 2>/dev/null; then
          TMOUT_VAL=$_tmout_input
        else
          TMOUT_VAL=$DEFAULT_TMOUT
          [ -n "$_tmout_input" ] && \
            echo -e " ${YELLOW}잘못된 입력 — 기본값 ${DEFAULT_TMOUT}초로 설정합니다.${RESET}"
        fi
        echo ""
      ...
      if [[ "$_yn_u28" != [Yy] ]]; then
        ...
        if systemctl list-unit-files firewalld.service &>/dev/null; then
          ...
          if systemctl is-active firewalld 2>/dev/null | grep -q '^active'; then
            echo "   firewalld 활성화 완료"
          else
            iptables -A INPUT -m state --state ESTABLISHED,RELATED -j ACCEPT
            iptables -A INPUT -p tcp --dport 22 -j ACCEPT
            iptables -A INPUT -i lo -j ACCEPT
            iptables -A INPUT -j DROP
            echo "   firewalld 활성화 실패 — iptables 기본 룰 적용 완료"
          fi
        else
          iptables -A INPUT -m state --state ESTABLISHED,RELATED -j ACCEPT
          iptables -A INPUT -p tcp --dport 22 -j ACCEPT
          iptables -A INPUT -i lo -j ACCEPT
          iptables -A INPUT -j DROP
          echo "   firewalld 미설치 — iptables 기본 룰 적용 완료"
        fi
        echo ""
        _lbl_result
...
do_fix "U-54" "(중) 암호화되지 않는 FTP 서비스 비활성화" \
  "ss -tlnp 2>/dev/null | grep ':21 ' || echo 'FTP 비활성 (양호)'" \
  ...
  "ss -tlnp 2>/dev/null | grep ':21 ' || echo 'FTP 비활성 (VERIFY_OK)'" \
  "VERIFY_OK"
...
do_fix "U-58" "(중) 불필요한 SNMP 서비스 구동 점검" \
  "systemctl is-active snmpd 2>/dev/null; ss -ulnp 2>/dev/null | grep ':161 ' || echo 'SNMP 비활성 (양호)'" \
  "systemctl stop snmpd 2>/dev/null; systemctl disable snmpd 2>/dev/null; systemctl mask snmpd 2>/dev/null" \
  "ss -ulnp 2>/dev/null | grep ':161 ' || echo 'SNMP 비활성 (VERIFY_OK)'" \
  "VERIFY_OK"
  ...
  _flush_header
    if [ $_vs -eq 1 ]; then
      ...
      if [[ "$_yn_u62" != [Yy] ]]; then
        ...
        echo "   ******************************************************************"
        echo ""
        echo -e " ${YELLOW}※ y = 기본 문구 사용, n = 직접 입력${RESET}"
...
# U-64 보안 패치 (패키지 관리자 및 구독 등록 여부에 따라 분기)
{
  ...
  _flush_header
    if [ $_vs -eq 1 ]; then
      ...
      echo ""
      echo -e " ${YELLOW}※ y = apt-get update && upgrade 적용, n = 건너뜀${RESET}"
      _read_yn _yn64apt " 조치하시겠습니까? (y/n): "
      case "$_yn64apt" in
        [Yy])
          _lbl_during
          echo -e "   ${CYAN}→${RESET} apt-get update && apt-get upgrade -y 실행"
          ...
          echo ""
          _mark_fixed "U-64" "apt-get update && upgrade 실행" ;;
        *)
      ...
      echo -e " ${YELLOW}※ y = 보안 패치 적용, n = 건너뜀${RESET}"
