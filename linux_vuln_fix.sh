  ...
  case "$id" in
    ...
    U-30)
      ...
      for F in /etc/profile /etc/bashrc /etc/bash.bashrc /etc/login.defs /etc/profile.d/*.sh; do
        ...
        if [[ "$V" =~ ^0*[0-7]{3,4}$ ]] && [ $(( (8#$V) & (8#022) )) -eq $(( 8#022 )) ]; then
          return 1
        fi
...
# Include 파일까지 반영한 전체 목록은 조치 직전 "Apache 설정 파일 탐색" 단계(_APACHE_CONFIGS
# 재구성)에서 채워진다. 프리체크 단계에서는 최소한 메인 설정 파일만이라도 검사 대상에
# 넣어 U-68/70/72/73/75가 항상 "해당없음"으로만 뜨는 일이 없게 한다.
[ -n "$_FIX_APACHE_CONF" ] && _APACHE_CONFIGS=("$_FIX_APACHE_CONF")
...
# ── 구분선 함수 조기 정의 ─────────────────────────────────────────────────────
# 구분선은 이 3개만 사용한다 — 항목 내부에서 직접 echo로 선을 찍지 않는다.
#   _div_item  : 굵은 구분선 — 항목(카드) 시작 시 1번만 (_item_header 안에서만 호출)
#   _div_sec   : 얇은 구분선 — [현재 상태]/[조치 중]/[조치 결과] 같은 섹션 제목에는 쓰지
#                않는다(색상만 사용). 한 항목 안에서 서로 다른 하위 블록을 나눌 때만
#                (_lbl_subdiv 경유) 선택적으로 사용한다.
#   _div_blank : 빈 줄
_div_item() {
  echo -e "${WHITE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}"
}
...
_sec() {
  ...
  case "$1" in
    ...
    verify) echo -e " ${BOLD}${CYAN}[최종 검증]${RESET}" ;;
...
_apache_reload_guard() {
  ...
  echo ""
  echo "✓ Apache 설정 문법 검사"
  if [ -z "$test_cmd" ]; then
    _APACHE_LAST_CONFIGTEST="명령 없음"
    _APACHE_LAST_RELOAD="생략 (수동 확인 필요)"
    echo "  configtest : ${_APACHE_LAST_CONFIGTEST}"
    return 1
  fi

  if $test_cmd 2>&1 | grep -q 'Syntax OK'; then
    _APACHE_LAST_CONFIGTEST="OK"
    echo "  configtest : OK"
    echo ""
    echo "✓ Apache 서비스 Reload"
    systemctl reload httpd 2>/dev/null || systemctl reload apache2 2>/dev/null || true
    _APACHE_LAST_RELOAD="완료"
    _LAST_RELOAD_STATUS="성공"
    echo "  reload : 완료"
    return 0
  else
    _APACHE_LAST_CONFIGTEST="실패"
    echo "  configtest : 실패 — 백업에서 복구합니다 (수정된 파일 ${#files[@]}개 전체)"
    $test_cmd 2>&1 | sed 's/^/    /'
    local _restored=0
    for _cf in "${files[@]}"; do
      [ -z "$_cf" ] && continue
      ...
      if [ -n "$latest_bak" ] && [ -f "$latest_bak" ]; then
        cp "$latest_bak" "$_cf"
        echo "  복구 완료 : $latest_bak → $_cf"
        _restored=1
      fi
    done
    if [ $_restored -eq 0 ]; then
      _APACHE_LAST_RELOAD="복구 실패 — 백업 없음, 수동 확인 필요"
      _LAST_RELOAD_STATUS="실패"
      echo "  ! 백업 파일 없음 — 수동 복구 필요"
    else
      # 복구 후 재검증 — 모든 파일을 되돌렸는데도 여전히 문법 오류면 다른 원인이 있을 수 있음
      if $test_cmd 2>&1 | grep -q 'Syntax OK'; then
        _APACHE_LAST_RELOAD="설정 복구됨 (변경 취소, 재검증 통과)"
        _LAST_RELOAD_STATUS="실패"
        echo "  복구 후 재검증 통과"
      else
        _APACHE_LAST_RELOAD="복구 후에도 문법 오류 — 수동 확인 필요"
        _LAST_RELOAD_STATUS="실패"
        echo "  ! 복구 후에도 여전히 문법 오류 — 수동 확인 필요"
      fi
    fi
    return 1
  fi
...
do_fix() {
  ...
  case "$yn" in
    [Yy])
      ...
      _sec during
      {
        echo "----- [$id] 실행 명령 ($(date '+%Y-%m-%d %H:%M:%S')) -----"
        echo "$fix_cmd"
        echo "-----------------------------------------------------"
      ...
      cat "$_fix_tmp" >> "$FIX_HISTORY_FILE" 2>/dev/null
      if [ -s "$_fix_tmp" ]; then
        sed 's/^[^[:space:]]/   &/' "$_fix_tmp"
      else
        # postconf/sed/chmod 등 무출력 명령이면 헤더만 비어 보이므로, 사용자가
        # 이해할 수 있는 체크리스트 문구로 진행 상태를 남긴다.
...
"
   ...
   echo "[참고]"
   echo ""
   echo "허용 목록 상세는 로그에 기록했습니다."
   echo "→ ${FIX_HISTORY_FILE}"
   {
     echo "----- [U-23] 허용 목록 상세 ($(date "+%Y-%m-%d %H:%M:%S")) -----"
...
/usr/bin/vmware-user-suid-wrapper"
   _u23_targets=()
   while IFS= read -r f; do
     echo "$ALLOWED" | tr " " "\n" | grep -qxF "$f" || _u23_targets+=("$f")
   done < <(find / -xdev \( -perm -4000 -o -perm -2000 \) -type f 2>/dev/null)
   echo "   대상 파일 : ${#_u23_targets[@]}개"
   echo "   처리 방식 : 허용 목록 외 SUID/SGID 제거"
   echo "   상세 로그 : ${FIX_HISTORY_FILE}"
   echo ""
   for f in "${_u23_targets[@]}"; do
     _before=$(ls -l "$f" 2>/dev/null | awk "{print \$1, \$3, \$4}")
     if chmod u-s,g-s "$f" 2>/dev/null; then
       _after=$(ls -l "$f" 2>/dev/null | awk "{print \$1, \$3, \$4}")
       printf "   ✓ %s\n" "$f"
       printf "       전: %s\n" "$_before"
       printf "       후: %s\n\n" "$_after"
     else
...
do_fix "U-48" "(중) expn, vrfy 명령어 제한" \
  ...
  "if ! command -v postconf &>/dev/null; then
     ...
     echo \"  \${before_out} → disable_vrfy_command = yes\"
     if postconf -e 'disable_vrfy_command = yes'; then
       echo ''
       echo '✓ Postfix 설정 문법 확인'
       _u48_chk=\$(postfix check 2>&1)
       if [ -n \"\$_u48_chk\" ]; then echo \"\$_u48_chk\" | sed 's/^/  /'; else echo '  이상 없음'; fi
       echo ''
...
echo ""
_div_thick
echo -e "${BOLD}Apache 환경 분석${RESET}"
echo ""
echo "U-68 ~ U-76에서 사용할 Apache 설정 파일을 자동 분석합니다."
echo ""
...
${conf}"
  ...
  # ── 전체 설정 파일 목록 구성 ────────────────────────────────────────────────
  # _APACHE_CONFIGS: check_still_vuln()과 _fix_apache_grep()/_apache_backup_all()이
  # 공유하는 배열. 메인 conf + 재귀적으로 찾은 Include 파일 전체를 담는다.
  _FIX_APACHE_ALL_CONFS="$_FIX_APACHE_CONF"
  _APACHE_CONFIGS=("$_FIX_APACHE_CONF")
  ...
  if [ ${#_apache_include_files[@]} -gt 0 ]; then
    echo -e "✓ Include Files (${#_apache_include_files[@]}개)"
    for _f in "${_apache_include_files[@]}"; do
      echo -e "  ${_f}"
    done
  else
    echo "✓ Include Files (없음)"
  fi
fi
...
# _apache_backup_all — _APACHE_CONFIGS에 담긴 모든 Apache 설정 파일을 조치 직전
# 시점의 .bak.<타임스탬프>로 개별 백업한다. _apache_reload_guard가 나중에 같은
# 파일에 대해 "가장 최신 .bak.*"를 찾아 복구하므로 네이밍 규칙을 맞춰야 한다.
_apache_backup_all() {
  [ "${_APACHE_SKIP:-1}" -eq 1 ] && return 0
  echo "✓ Apache 설정 파일 백업"
  local _ts; _ts=$(date +%Y%m%d_%H%M%S)
  for _cf in "${_APACHE_CONFIGS[@]}"; do
    [ -f "$_cf" ] && cp -p "$_cf" "${_cf}.bak.${_ts}" 2>/dev/null
  done
}

# _apache_result_summary <완료 문구>
# fix_cmd 실행 중 채워진 _APACHE_LAST_CHANGED를 [조치 결과]에서 사람이 읽기
# 좋은 요약으로 보여준다. (do_fix가 이후 자동으로 [최종 검증]을 붙이므로
# 여기서는 [조치 결과] 내용만 담당한다 — ConfigTest/Reload 결과는
# _apache_reload_guard가 _LAST_RELOAD_STATUS에 남기면 [최종 검증]에서
# 자동으로 표시된다.)
_apache_result_summary() {
  local _label="$1"
  echo "변경 파일 : ${#_APACHE_LAST_CHANGED[@]}개"
  echo ""
  local _cf
  for _cf in "${_APACHE_LAST_CHANGED[@]}"; do
    echo "✓ $_cf"
  done
  echo ""
  echo "적용 내용 : ${_label} 완료"
  echo ""
  echo "configtest : ${_APACHE_LAST_CONFIGTEST}"
}
...
do_fix "U-68" "(상) Apache 디렉터리 리스팅 제거" \
  ...
  "_apache_backup_all
   _APACHE_LAST_CHANGED=()
   _u68_targets=()
   for _cf in \"\${_APACHE_CONFIGS[@]}\"; do
     [ -f \"\$_cf\" ] || continue
     grep -iE 'Options.*Indexes' \"\$_cf\" | grep -qiv '\\-Indexes' && _u68_targets+=(\"\$_cf\")
   done
   echo \"\"
   echo \"✓ 디렉터리 리스팅(Indexes) 설정 검색\"
   echo \"  대상 : \${#_u68_targets[@]}개 파일\"
   echo \"\"
   echo \"✓ Indexes 옵션 제거\"
   for _cf in \"\${_u68_targets[@]}\"; do
     ...
     echo \"  \$_cf\"
     _APACHE_LAST_CHANGED+=(\"\$_cf\")
   done
   _apache_reload_guard \"\${_APACHE_CONFIGS[@]}\"" \
  "_o=\$(_fix_apache_grep 'Options.*Indexes' | grep -iv '\-Indexes'); if [ -z \"\$_o\" ]; then _apache_result_summary 'Indexes 옵션 제거'; echo 'VERIFY_OK'; else echo \"\$_o\"; fi" \
  "VERIFY_OK"
...
do_fix "U-70" "(상) Apache 상위 디렉터리 접근 금지" \
  ...
  "_apache_backup_all
   _APACHE_LAST_CHANGED=()
   echo \"\"
   echo \"✓ 상위 디렉터리(AllowOverride) 설정 검색 및 적용\"
   for _cf in \"\${_APACHE_CONFIGS[@]}\"; do
     [ -f \"\$_cf\" ] || continue
  ...
  "_o=\$(for _cf in \$_FIX_APACHE_ALL_CONFS; do
   ...
   done); if [ -n \"\$_o\" ]; then _apache_result_summary 'AllowOverride None 적용'; echo \"\$_o\"; else echo '검증 실패'; fi" \
  "VERIFY_OK"
...
do_fix "U-72" "(상) Apache 심볼릭 링크 사용 금지" \
  ...
  "_apache_backup_all
   ...
   for _cf in \"\${_APACHE_CONFIGS[@]}\"; do
     [ -f \"\$_cf\" ] || continue
     grep -iE 'Options.*FollowSymLinks' \"\$_cf\" | grep -qiv '\-FollowSymLinks' && _u72_targets+=(\"\$_cf\")
   done
   echo \"\"
   echo \"✓ 심볼릭 링크(FollowSymLinks) 설정 검색\"
   echo \"  대상 : \${#_u72_targets[@]}개 파일\"
   echo \"\"
   echo \"✓ FollowSymLinks 옵션 제거\"
   for _cf in \"\${_u72_targets[@]}\"; do
   ...
   _apache_reload_guard \"\${_APACHE_CONFIGS[@]}\"" \
  "_o=\$(_fix_apache_grep 'Options.*FollowSymLinks' | grep -iv '\-FollowSymLinks'); if [ -z \"\$_o\" ]; then _apache_result_summary 'FollowSymLinks 옵션 제거'; echo 'VERIFY_OK'; else echo \"\$_o\"; fi" \
  "VERIFY_OK"
...
do_fix "U-73" "(상) Apache 파일 업로드/다운로드 용량 제한" \
  ...
  "_apache_backup_all
   _APACHE_LAST_CHANGED=()
   echo \"\"
   echo \"✓ 업로드 용량 제한(LimitRequestBody) 설정 검색\"
   if [ -n \"\$_FIX_APACHE_CONF\" ] && ! _fix_apache_grep 'LimitRequestBody' | grep -qE '^[0-9]+\$'; then
     echo \"  대상 : \$_FIX_APACHE_CONF (미설정)\"
     echo \"\"
     echo \"✓ LimitRequestBody 10485760(10MB) 추가\"
     {
       echo ''
       echo '# KISA U-73: 파일 업로드/다운로드 제한'
       echo 'LimitRequestBody 10485760'
     } >> \"\$_FIX_APACHE_CONF\"
     echo \"  \$_FIX_APACHE_CONF\"
     _APACHE_LAST_CHANGED+=(\"\$_FIX_APACHE_CONF\")
   else
     echo \"  이미 설정되어 있어 변경하지 않습니다.\"
   fi
   _apache_reload_guard \"\$_FIX_APACHE_CONF\"" \
  "_o=\$(_fix_apache_grep 'LimitRequestBody' | head -2 2>/dev/null); if [ -n \"\$_o\" ]; then _apache_result_summary 'LimitRequestBody 10485760 적용'; echo \"\$_o\"; else echo '설정 없음'; fi" \
  "LimitRequestBody"
...
do_fix "U-75" "(중) Apache 웹 서비스 정보 숨김" \
  ...
  "_apache_backup_all
   ...
   if [ -n \"\$_FIX_APACHE_CONF\" ] && [ -f \"\$_FIX_APACHE_CONF\" ]; then
     echo \"\"
     echo \"✓ 서버 정보 노출(ServerTokens/ServerSignature) 설정 적용\"
  ...
  "_o=\$(_fix_apache_grep 'ServerTokens|ServerSignature' | head -4); _apache_result_summary 'ServerTokens Prod, ServerSignature Off 설정'; echo \"\$_o\"" \
  "Prod.*Off|Prod|Off"
